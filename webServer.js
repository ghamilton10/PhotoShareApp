...
app.post('/photos/new', function (req, res) {
  ...
  processFormBody(req, res, function (err) {
    if (err || !req.file) {
      console.log("Error produced, check image file");
      return res.status(400).send("Error: No file uploaded or file upload error.");
    }
    ...
    // eslint-disable-next-line consistent-return
    fs.writeFile("./images/" + filename, req.file.buffer, function (writeErr) {
      if (writeErr) {
        console.log("Error uploading image to directory");
        return res.status(500).send("Error: Unable to upload image to directory.");
      }

      // Create the photo object only after the file is successfully written
      const newPhoto = new Photo({
        file_name: filename,
        date_time: timestamp,
        user_id: req.body.user_id, // Assuming user_id comes from the body. Adjust if it's from somewhere else.
        comments: []
      });
      ...
      newPhoto.save()
          ...
          .catch((saveErr) => {
            console.log(saveErr);
            res.status(500).send("Error: Unable to save photo information.");
          });
    });
  });
});
